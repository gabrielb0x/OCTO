...
- **Gestion des publicités** : les publicités personnalisées et l'historique des publicités s'activent et se coupent **dans ton compte** (`bazaar_personalization_enabled`, `bazaar_history_enabled`), le profil publicitaire que ChatGPT garde pour toi se **supprime** (`DELETE bazaar/profile`), et un compte gratuit peut **échanger ses publicités contre moins de messages** (`free_ads_opt_out`), de façon réversible. OCTO, lui, n'affiche aucune publicité et n'envoie rien aux annonceurs.
- **Mémoire** : « Référencer les souvenirs enregistrés » et « Référencer l'historique des chats » sont des interrupteurs enregistrés dans ton compte (`sunshine`, `moonshine`), et un souvenir se supprime d'un balayage.
...
- **Aucune télémétrie**, aucun outil d'analyse, **aucune dépendance tierce**. Et même si quelque chose essayait : OCTO **bloque toute requête vers les adresses de télémétrie** de ChatGPT et de Codex (`chatgpt.com/ces/…`, `api.oaistatsig.com`, `ab.chatgpt.com`, Datadog, Sentry…) avant qu'elle ne quitte l'iPhone. La liste est dans Réglages → Confidentialité → Télémétrie bloquée.
...
- Ton **adresse e-mail et ton numéro de téléphone restent cachés** derrière des points dans les Réglages : une pression les montre 30 secondes, ou bien ils ne se montrent jamais, ou seulement quand l'écran n'est pas enregistré, recopié ni partagé, ou leurs lignes **disparaissent complètement des Réglages** — de quoi ouvrir les Réglages en live sans rien dévoiler.
...
- **Plusieurs comptes** peuvent être connectés en même temps : chacun a son entrée dans le trousseau et son dossier de chats, et Réglages → Comptes permet de passer de l'un à l'autre ou d'en déconnecter un. Déconnecter un compte révoque sa session auprès d'OpenAI.

Avec la même session, OCTO lit ton compte sur `chatgpt.com/backend-api` : profil, abonnement, réglages, instructions personnalisées, personnalité, mémoire, chats, projets, statut de l'âge, réglages d'entraînement et réglages publicitaires. Tes instructions, ta personnalité, « Améliorer le modèle pour tout le monde », les réglages de mémoire et ceux des publicités peuvent être modifiés depuis l'app, avec les mêmes requêtes que le site. Les réglages propres aux apps officielles (plugins, contrôle à distance) expliquent ce qu'ils font et ouvrent ChatGPT.

Les réponses sont générées par le backend Codex de ton forfait et décomptées de ses **limites d'utilisation Codex** (visibles dans Réglages → Utilisation de Codex). Les messages écrits dans OCTO ne sont donc pas ajoutés à tes chats sur chatgpt.com : ils restent sur l'appareil, et une mention l'indique dans les chats venant du compte.
